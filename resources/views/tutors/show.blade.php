<x-app-layout>
    <style>
        ...
        .tutor-avatar {
            width: 120px;
            /* Size thora bara kiya hai */
            height: 120px;
            border-radius: 24px;
            /* Squircle look (iPhone icons jesa) */
            background: #f8fafc;
        ...
        .tutor-avatar img {
            ...
            height: 100%;
            object-fit: cover;
            /* Image ko stretch nahi hone dega */
        }
                ...
                <div style="position:relative;z-index:1;">
                    <div style="display:flex;align-items:flex-start;gap:1.5rem;flex-wrap:wrap;">
                        ...
                        <div style="text-align:right;flex-shrink:0;">
                            <p
                                style="color:rgba(255,255,255,0.6);font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;">
                                Hourly Rate</p>
                            <p style="color:#fff;font-size:2.2rem;font-weight:900;line-height:1;">
                                ${{ $tutor->tutorProfile->hourly_rate ?? 0 }}</p>
            ...
            <div x-data="{ openStore: false }" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    <div class="section-card">
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:0.6rem;">
                            ...
                            @forelse($skills as $skill)
                                ...
                                @endif
                            @empty
                                <p style="color:#94a3b8;font-size:0.85rem;font-style:italic;">General subjects and
                                    mentoring.</p>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div id="all-reviews" class="section-card scroll-mt-24">
                        <h3 class="section-title" style="margin-bottom:1.5rem;"><span class="section-dot"></span>
                            Student Reviews</h3>
                        <div style="display:flex;flex-direction:column;gap:0.875rem;">
                            @forelse($tutor->reviews as $review)
                                <div class="review-card">
                                    <div class="reviewer-avatar">
                                        {{ strtoupper(substr($review->student->name ?? 'S', 0, 1)) }}</div>
                                    <div style="flex:1;">
                                        <div
                                            style="display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem;flex-wrap:wrap;">
                                            <div>
                                                <p style="font-weight:700;color:#0f172a;font-size:0.9rem;">
                                                    {{ $review->student->name }}</p>
                                                <p style="font-size:0.75rem;color:#94a3b8;margin-top:0.1rem;">
                                        ...
                                        @if($review->comment)
                                            <p
                                                style="margin-top:0.6rem;color:#475569;font-size:0.88rem;font-style:italic;line-height:1.6;">
                                                "{{ $review->comment }}"
                                            </p>
                                        @endif
                                    </div>
                            ...
                            @empty
                                <div style="text-align:center;padding:3rem 0;color:#94a3b8;font-style:italic;">No reviews
                                    yet.</div>
                            @endforelse
                        </div>
                    </div>
                            ...
                            <div @click.outside="showBookingModal = false">
                                <div class="modal-box">
                                    <div style="text-align:center;margin-bottom:1.75rem;">
                                        <div
                                            style="width:52px;height:52px;background:linear-gradient(135deg,#4f46e5,#7c3aed);border-radius:14px;display:flex;align-items:center;justify-content:center;margin:0 auto 1rem;">
                                        ...
                                        <h3
                                            style="font-size:1.25rem;font-weight:800;color:#0f172a;font-family:'Lexend',sans-serif;">
                                            Request a Booking</h3>
                                        <p style="color:#94a3b8;font-size:0.83rem;margin-top:0.25rem;">with
                                            {{ $tutor->name }}</p>
                                    </div>
                                    <form action="{{ route('bookings.store') }}" method="POST"
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="subject" placeholder="e.g. Python for Beginners"
                                        ...
                                        <div>
                                            ...
                                            <textarea name="message"
                                                placeholder="Any specific topics you want to cover?" class="modal-input"
                ...
                <div x-show="openStore" x-cloak class="modal-overlay" style="display:none;" x-transition.opacity>
                    <div class="modal-box" @click.outside="openStore = false">
                        <div
                            ...
                            <h3
                                style="font-size:1.15rem;font-weight:800;color:#0f172a;font-family:'Lexend',sans-serif;">
                                Digital Store</h3>
                            <button @click="openStore = false" class="close-btn"
                                style="background:#f1f5f9;color:#64748b;">×</button>
                        </div>
                        <p style="color:#94a3b8;text-align:center;padding:3rem 0;font-style:italic;">No items in store
                            yet.</p>
                    </div>
                </div>
            ...
            <div style="flex:1;padding:1.5rem 2rem;overflow-y:auto;display:flex;flex-direction:column;gap:1rem;">
                @forelse($tutor->reviews as $review)
                    <div class="review-card">
                        ...
                        <div style="flex:1;">
                            <div
                                style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:0.4rem;">
                                <div>
                                    <p style="font-weight:700;color:#0f172a;font-size:0.9rem;">{{ $review->student->name }}
                                    </p>
                                    <p style="font-size:0.75rem;color:#94a3b8;">{{ $review->created_at->diffForHumans() }}
                                    </p>
                                </div>
                                <div class="stars">
                                    @for($i = 1; $i <= 5; $i++){{ $i <= $review->rating ? '★' : '☆' }}@endfor
                            ...
                            @if($review->comment)
                                <p style="margin-top:0.6rem;color:#475569;font-size:0.87rem;font-style:italic;">
                                    "{{ $review->comment }}"</p>
                            @endif
                        </div>
                    </div>
                ...
                @if(auth()->user()->role === 'student')
                    ...
                    @if(!$alreadyReviewed)
                        <div style="border-top:1px solid #e2e8f0;padding:1.5rem 2rem;background:#f8fafc;">
                            <h4
                                style="font-weight:800;font-size:0.95rem;color:#0f172a;margin-bottom:1rem;font-family:'Lexend',sans-serif;">
                            ...
                            <form action="{{ route('reviews.store', $tutor->id) }}" method="POST">
                                ...
                                <textarea name="comment" rows="3" class="modal-input"
                                    style="height:80px;resize:none;margin-bottom:1rem;"
                                    placeholder="Share your honest experience..."></textarea>
